...
class FltDisp(Display):
    ...
    def __init__(self, parent=None, args=None, ui_filename="FltDisp.ui"):
        ...
        self.ui.Plot2.addWidget(LowerPlot)                      # Write the LowerPlot Figure as a widget to the
    ...
    def setGOVal(self,ac, ac2):
        ...
        if EndTimeForArchiver >= StTimeForArchiver:   # Checks that the End time is after the Start time
            ...
            if (cmNumUpper!= 0 and HUpper != 0) or (cmNumLower != 0 and HLower != 0):  # This checks if more than one
                ...
                if self.ui.RfFaults.isChecked():                        #If no CM selected and if RfFaults radiobutton
                     ...
                     #
                     self.cmFaults(ac,ac2,StTimeForArchiver,EndTimeForArchiver)
                else:                         # Otherwise call the function, 'cmStats' which
                                              # calls the arPull module to get faults and recovery times for all modules
                     self.cmStats(ac,ac2,StTimeForArchiver,EndTimeForArchiver)
                ...
                if cmNumUpper != 0:                   # Need to check that upper CM selector != 0 before reading value
                     ...
                     OutTextUpper = "H" + str(HUpper) # if so, grab that value
                ...
                if self.ui.RfFaults.isChecked():       #If the RfFaults radiobutton is selected, call cavFaults function
                     self.cavFaults(ac, ac2, OutTextUpper,OutTextLower,StTimeForArchiver, EndTimeForArchiver)
                else:                                  #If the RF Stats radiobutton was selected, call cavStats function
...
#
    def cmFaults(self,ac,ac2,StTim,EnTim):             # This is function to plot cryomodule faults
...
# StDTop, StDLow are the arrays for the Standard Deviation between the number of cavity faults with in the CM.
                                                       #
        CmNumTop, CmNumLow, numFltsTop, numFltsLow, StDTop, StDLow = arPull.cmFlts(StTim, EnTim)
# call the function in arPull module with Start Time for Data (StTim) and the End Time for Data (EnTim).
    ...
    def cmStats(self,ac,ac2,StTim,EnTim):
        # called from the setGOVal function after it branches to determine correct plot function to call
...
# Call to function CmStats in arPull module giving Start and end time.  It queries the archiver and returns arrays
                                                       # for display by ui
        CmNumTop, CmNumLow, MeanRecTop, MeanRecLow, StDevRec, StDevRec2 = arPull.CmStats(StTim, EnTim)
# CmNumTop and CmNumLow are the arrays for the CMs labels for the upper and lower plots.  Nom is 1-15 top (15@1.3's and
                                                       #  2@3.9's) and 16 through 35 on the lower plot (L3B).
...
# array for the plot labels, and a list "reS" a dict with an entry for the number of each type of fault for each cavity
...
# The fabric variables are need since matplotlib wants an offset from the baseline to write multiple bars
...
#
    ...
    # CavFaults Upper and Lower are arrays of faults (see plotOne comments above) to be parsed and displayed
        CavNumTop, CavNumLower, CavFaultsUpper, CavFaultsLower = arPull.cavFaults(OutTextUpper, OutTextLower, StTim, EnTim)
    ...
    def cavStats(self,ac,ac2,OutTextUpper,OutTextLower,StTim,EnTim):
        ...
        # called from the setGOVal function after it branches to determine correct plot function to call
        #
        CavNumTop,CavNumLower,cavRec,cavRecStDev, cavRec2,cavRecStDev2 = arPull.cavStats(OutTextUpper, OutTextLower, StTim, EnTim)
