...
#************************
    aaaa=list(results2.keys())

    for n in range(0, 37):
            CMtot = []
            for i in range(0, 8):
                tut=aaaa[i+n*8]
                CMtot.append(results2[tut])
...
#############
    ...
    for cav in aaaa:
        ...
        xx = numpy.array(dd)
        cavTot.append(numpy.sum(dd)/60.0)
    #print(cavTot)
    #print(len(cavTot))
    for n in range(0,37):
        CMtot=[]
...
if __name__ == "__main__":
    ...
    #e, f, r1,r2,r3,r4  = cavStats("4","27",datetime.time,datetime.time)
    e,f,loFlts,hiFlts, LoStd, HiStd = cmFlts(datetime.time,datetime.time)
    print("e=",e,"\r\n", loFlts,"\r\n",LoStd)
